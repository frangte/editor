...
exports.lineIndent = (state, dispatch, view) => {
    if (!isSelectionEntirelyInsideMathBlock(state))
        return false;
    return indent(state, dispatch);
};
exports.lineUndent = (state, dispatch, view) => {
    if (!isSelectionEntirelyInsideMathBlock(state))
        return false;
    return undent(state, dispatch);
};
exports.newlineIndent = (state, dispatch, view) => {
    if (!isSelectionEntirelyInsideMathBlock(state))
        return false;
    return insertNewlineWithIndent(state, dispatch);
};
exports.deleteMathBlock = (state, dispatch, view) => {
    if (!isSelectionEntirelyInsideMathBlock(state))
        return false;
